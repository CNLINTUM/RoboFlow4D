...
def parse_args():
    ...
    p.add_argument('--box_thresh', type=float, default=0.30)   # 0.35 0.35
...
def run_gsam2_on_first_frame(grounding_model, sam2_predictor: SAM2ImagePredictor,
    ...
    point_coords_b, point_labels_b = add_pos_neg_points_libero_10(boxes_xyxy_pixels, h, w)
            ...
            with h5py.File(hdf5_file, "r") as f:
                ...
                with h5py.File(out_h5_path, "w") as fout:
                    ...
                    for demo_id in demo_ids:
                        ...
                        ee_states = f[f"data/{demo_id}/obs/ee_states"]
                        gripper_states = f[f"data/{demo_id}/obs/gripper_states"]
                        robot_states = np.concatenate([ee_states, gripper_states], axis=-1)

                        actions = f[f"data/{demo_id}/actions"]
                        dones = f[f"data/{demo_id}/dones"]

                        if args.save_wrist:
                            wrist_frames = f[f"data/{demo_id}/obs/eye_in_hand_rgb"]
                        ...
                        full_frames = f[f"data/{demo_id}/obs/{args.image_key}"][:]   # [T_full, H, W, C]
                        full_frames = full_frames[:, ::-1, ::-1, :].copy()
