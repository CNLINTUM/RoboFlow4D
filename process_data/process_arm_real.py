...
def _repeat_by_stride(arr: np.ndarray, stride: int, T_full: int) -> np.ndarray:
    """
    Repeat or expand the time dimension by stride until T_full.
    For stride=3: t=0 -> 0,1,2; t=1 -> 3,4,5; ...
    """
    T_s = arr.shape[0]
...
def _interp_by_stride(arr: np.ndarray, stride: int, T_full: int) -> np.ndarray:
    """
    Linearly interpolate the time dimension to T_full.
    Downsampled source frames are assumed to lie at [0, stride, 2*stride, ...].
    """
    T_s = arr.shape[0]
    if T_s == 1:
        return np.repeat(arr, T_full, axis=0)

    t_src = np.arange(T_s, dtype=np.float32) * float(stride)  # [T_s]
    t_tgt = np.arange(T_full, dtype=np.float32)               # [T_full]

    orig_shape = arr.shape
    arr_flat = arr.reshape(T_s, -1)                           # [T_s, D]
...
def parse_args():
    ...
    # Data
    p.add_argument('--image_key', default='image')
...
def add_pos_neg_points(input_boxes, h, w):
    POS_POINTS = [(160, 32)]
    NEG_POINTS = [(174, 64), (160, 10), (160, 10),
...
def add_pos_neg_points_libero_10(input_boxes, h, w):
    POS_POINTS = [(128, 32), (128, 40), (120, 40), (136, 40), (116, 44), (140, 44), (128, 24), (128, 16), (116, 54), (140, 54)]
    NEG_POINTS = [(128, 90), (120, 90), (136, 90), (116, 90), (128, 74), (124, 74), (132, 74), (96, 4), (160, 4), (94, 2), (162, 2), (98, 6), (158, 6), (96, 6), (160, 6), (92, 2), (164, 2),(158, 4), (156, 4), (154, 4), (152, 4),
                  (input_boxes[0, 0], input_boxes[0, 1]), (input_boxes[0, 0], input_boxes[0, 3]),
                  (input_boxes[0, 2], input_boxes[0, 1]), (input_boxes[0, 2], input_boxes[0, 3])]
...
def run_gsam2_on_first_frame(grounding_model, sam2_predictor: SAM2ImagePredictor,
    ...
    # Shift gripper boxes upward to focus SAM2 on the end-effector region.
    def _is_gripper(name: str) -> bool:
        s = str(name).strip().lower().rstrip('.')
        return s == 'robotic gripper' or s == 'white robotic gripper' or s == 'black robotic gripper' or s.startswith('robotic ')
    ...
    masks = torch.from_numpy(masks)  # [B,K,H,W] or [B,1,H,W].
    if masks.dim() == 4:
        if multimask_output:
...
def run_spatial_tracker(predictor: Predictor, vggt_front: VGGT4Track,
    ...
    # limit the number of tracking points to predictor.spatrack.track_num
    ...
    with amp_ctx:
        c2w_traj, intrs2, point_map, conf_depth, track3d_pred, track2d_pred, vis_pred, conf_pred, video_out = predictor.forward(
            ...
            support_frame=extrs.shape[0]-1, replace_ratio=0.2 # 0.2
        )
    ...
    for j in range(point_traj.shape[0]):
        ...
        # track2d_list.append(project_world_to_pixels(K, R, t, point_traj[j]))
...
# -------------------------
# Utils: safe overwrite HDF5 dataset
# -------------------------
def overwrite_dataset(h5group, name, data, **create_kwargs):
    """Create or overwrite a dataset under h5group[name]."""
    if name in h5group:
        del h5group[name]
...
# -------------------------
# Detect REAL dataset layout
# 1) root has observations/action
# 2) root has demo_*/ each demo has observations/action
# -------------------------
def list_real_demo_keys(f: h5py.File):
    # case A: single episode at root
    if "observations" in f and ("action" in f or "actions" in f):
        return [None]  # None => root demo
    # case B: multiple demos at root: demo_0, demo_1, ...
    demo_keys = sorted([k for k in f.keys() if k.startswith("demo_")])
    if demo_keys:
        # validate one
        g0 = f[demo_keys[0]]
        if "observations" in g0 and ("action" in g0 or "actions" in g0):
            return demo_keys
    return []


def make_default_dones(T_full: int):
    d = np.zeros((T_full,), dtype=np.int8)
    if T_full > 0:
        d[-1] = 1
    return d
...
def preprocess_inverse_xy(xy: np.ndarray, H0: int, W0: int,
                          mode: str = "crop", target_size: int = 518, keep_ratio: bool = False) -> np.ndarray:
    """
    Map xy coordinates from the preprocess_image output frame back to the
    original (H0, W0) image frame.
    """
    ...
    if mode == "crop":
        ...
        # resize: (H0,W0) -> (new_H,new_W)
        sx = W0 / float(new_W)
        sy = H0 / float(new_H)

        # Undo center crop when it was applied.
        if (keep_ratio is False) and (new_H > target_size):
            start_y = (new_H - target_size) // 2
            # Output coordinates -> resized coordinates.
            xy = xy.copy()
            xy[..., 1] = xy[..., 1] + float(start_y)

        # Resized coordinates -> original coordinates.
        xy[..., 0] = xy[..., 0] * sx
        xy[..., 1] = xy[..., 1] * sy
        return xy

    elif mode == "pad":
        # Pad mode: resize proportionally, then pad to (target, target).
        if W0 >= H0:
            new_W = target_size
            new_H = round(H0 * (new_W / W0) / 14) * 14
        else:
        ...
        # 1) Remove padding: output coordinates -> resized coordinates.
        xy = xy.copy()
        xy[..., 0] = xy[..., 0] - float(pad_left)
        xy[..., 1] = xy[..., 1] - float(pad_top)

        # 2) Resized coordinates -> original coordinates.
        sx = W0 / float(new_W)
        sy = H0 / float(new_H)
        xy[..., 0] = xy[..., 0] * sx
        xy[..., 1] = xy[..., 1] * sy
    ...
    for input_dir_str in args.input_dirs:
        ...
        for hdf5_file in tqdm(hdf5_files, desc=f"Processing files in {input_dir.name}"):
            ...
            if out_h5_path.exists():
                print(f"[SKIP] Already exists: {out_h5_path}")
                continue
            ...
            with h5py.File(hdf5_file, "r") as f:
                ...
                # Randomly choose one demo for the test video.
                ...
                with h5py.File(out_h5_path, "w") as fout:
                    ...
                    for demo_id in demo_ids:
                        ...
                        robot_states = f[f"data/{demo_id}/observations/state"]

                        actions = f[f"data/{demo_id}/actions"]

                        if args.save_wrist:
                            wrist_frames = f[f"data/{demo_id}/observations/wrist_image"]
                        ...
                        full_frames = f[f"data/{demo_id}/observations/{args.image_key}"][:]   # [T_full, H, W, C]
                        
                        if full_frames.shape[0] == 0:
                            print(f"  Skipping {base_name}: No frames."); continue

                        if args.image_size:
                            w, h = args.image_size
                            full_frames = np.stack(
                                [cv2.resize(frame, (w, h), interpolation=cv2.INTER_AREA)
                                for frame in full_frames],
                                axis=0
                            )

                        video_frames = full_frames[::stride]   # [T_s, H, W, C]
                        ...
                        if tracks and "track2d" in tracks and tracks["track2d"].size > 0:
                            ...
                            T_full = full_frames.shape[0]

                            if stride > 1:
                                tracks['frames_rgb'] = torch.from_numpy(full_frames).permute(0, 3, 1, 2).contiguous().float()

                                if 'point_traj' in tracks:
                                    tracks['point_traj'] = _interp_by_stride(
                                        tracks['point_traj'],   # [T_s, N, 3]
                                        stride=stride,
                                        T_full=T_full,
                                    )
                                if 'track2d' in tracks:
                                    tracks['track2d'] = _interp_by_stride(
                                        tracks['track2d'],      # [T_s, N, 2]
                                        stride=stride,
                                        T_full=T_full,
                                    )

                                if 'vis' in tracks:
                                    tracks['vis'] = _repeat_by_stride(
                                        tracks['vis'],          # [T_s, N]
                                        stride=stride,
                                        T_full=T_full,
                                    )
                                if 'intrs2' in tracks:
                                    tracks['intrs2'] = _repeat_by_stride(
                                        tracks['intrs2'],       # [T_s, 3, 3]
                                        stride=stride,
                                        T_full=T_full,
                                    )
                                if 'depths' in tracks:
                                    tracks['depths'] = _repeat_by_stride(
                                        tracks['depths'],       # [T_s, H, W]
                                        stride=stride,
                                        T_full=T_full,
                                    )
                                if 'extrinsics' in tracks:
                                    tracks['extrinsics'] = _repeat_by_stride(
                                        tracks['extrinsics'],   # [T_s, 4, 4]
                                        stride=stride,
                                        T_full=T_full,
                                    )
                                if 'c2w' in tracks:
                                    tracks['c2w'] = _repeat_by_stride(
                                        tracks['c2w'],          # [T_s, 4, 4]
                                        stride=stride,
                                        T_full=T_full,
                                    )
                                if 'vggt_hidden' in tracks:
                                    tracks['vggt_hidden'] = _repeat_by_stride(
                                        tracks['vggt_hidden'],  # [T_s, D] or [T_s,...]
                                        stride=stride,
                                        T_full=T_full,
                                    )
                            else:
                                tracks['frames_rgb'] = full_frames

                            add_spatracker_point_traj_alias(tracks, demo_grp)
                            
                            video_vis = torch.from_numpy(full_frames).permute(0, 3, 1, 2)  # [T_full, 3, H, W]
                            resize = T.Resize((518, 518))
                            video_vis = resize(video_vis).float()

                            for key, arr in tracks.items():
                                if key.startswith(
                                    (
                                        "point", "track", "vis", "grid", "frames",
                                        "intrs", "depth", "extr", "c2w", "p0_uv", "p0_world", "query_xy_t0", "vggt_hidden"
                                    )
                                ):
                                    demo_grp.create_dataset(
                                        key, data=to_np(arr), compression="gzip"
                                    )
